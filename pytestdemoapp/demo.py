...
class App(ctk.CTk):
    ...
    def dir_picker_frame(self):
        ...
        save_dir_label = ctk.CTkLabel(master=picker_frame, text="Save reports directory")
        save_dir_label.grid(row=3, column=1, padx=10, pady=5, sticky='W')
        self.save_dir_entry = ctk.CTkEntry(master=picker_frame, placeholder_text="Reports directory..",
                                           width=400)
        self.__set_save_dir_text(os.getcwd())
        self.save_dir_entry.grid(row=4, column=1, padx=10, pady=(10, 10), sticky="n")

        choose_dir_button = ctk.CTkButton(master=picker_frame, text="Choose dir", width=50,
                                          command=lambda: self.__browse_dir(True))
        choose_dir_button.grid(row=4, column=3, padx=(5, 10), pady=(10, 10), sticky="n")
    ...
    def __find_reports_file(self):
        os.chdir(self.save_dir_entry.get())
    ...
    def __browse_dir(self, is_save):
        directory = ctk.filedialog.askdirectory()
        if is_save:
            self.__set_save_dir_text(directory)
        else:
    ...
    def __set_save_dir_text(self, text):
        self.save_dir_entry.delete(0, ctk.END)
        self.save_dir_entry.insert(0, text)
